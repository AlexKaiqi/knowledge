...
const minimized = !/(?:\/tmp\/|module-cache|"Info"|"GoMod"|"Zip"|"Dir")/.test(serializedResult)
  && !/(?:cookie|token|credential)/i.test(serializedResult)
const probePassed = result.conformance.status === 'passed' && identityMatched && metadataMatched && authenticationMatched && isolationMatched && minimized
const snapshot = { schemaVersion: 'dsh.go-public-module-version-snapshot/v1', fixture: { expected }, ...result }
