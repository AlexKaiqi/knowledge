...
description: 只有通过可重复 probe 的能力及其可达知识才进入 canonical OKF bundle。
...
1. 平台、工具、信息源、数据集、服务或协议必须至少被一项已通过 probe 的 Capability 引用。
2. Platform、Information Source、Dataset、Service、Protocol 至少需要 sandbox 或 live 级验证；纯本地 Tool 可以用 local 级验证。
3. Capability 必须声明输入/输出 schema、访问方式、外部可见副作用以及新鲜的 probe report。
4. Concept 只有在被已准入 Capability 的输入或结果引用时才进入 canonical bundle。
5. 文档调研、理论能力、仅登记 provider、失败或过期 probe 都只属于候选区，不进入 `knowledge/`。
6. 一次 Git 变更只提升一个 subject 的一组紧密相关能力；Git commit 与内容哈希共同固定版本。
