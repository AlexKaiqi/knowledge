...
| `connector-definition.schema.json` | Capability 到同仓库隐藏执行入口的绑定；支持 Connector 默认 conformance 与 handler 级覆盖，以及 deterministic、agentic、hybrid、manual 执行 |
...
- `candidate` Connector/handler 可以接受审查和真实 probe，但不能被 canonical Capability 路由；准入按对应 Capability handler 的有效 conformance 判断，handler 未覆盖时继承 Connector 默认值。
