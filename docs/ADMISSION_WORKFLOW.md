...
1. Capability 至少声明一个被准入政策接受的产品结果域。
2. Subject 有可核验来源、状态和 freshness。
3. Capability 有稳定 ID/version、访问方式、授权要求和 effect。
4. 输入输出 Schema 能独立编译。
5. Connector 真实存在，入口可加载，handler 显式绑定 Capability。
6. Probe 可重复，有断言、预算和必要清理。
7. Report 为 `passed`、未过期，证据哈希匹配。
8. Platform/Source/Dataset/Service/Protocol 至少达到 sandbox 或 live 等级。
9. 相关 Concept 都从已准入 Capability 可达；无孤立知识。
